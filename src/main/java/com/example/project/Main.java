...
public class Main {
    ...
    public static ArrayList<String> moveBWords(ArrayList<String> wordList) {
        ...
        for (int i = 0; i < wordList.size(); i++) {
            if (wordList.get(i).toLowerCase().startsWith("b")) {
    ...
    public static ArrayList<Integer> removeDuplicates(ArrayList<Integer> intList) {
        for (int i = 0; i < intList.size(); i++) {
            if (i != intList.indexOf(intList.get(i))) {
                intList.remove(i);
                i--;
            }
        }
    ...
    // and after it, and those values are different from it. If the value is at the
    // end compare to the left and if its at the beginning compare to the right
    ...
    public static ArrayList<Integer> notAlone(ArrayList<Integer> list, int val) {
        ...
        int secondVal = list.get(1);
        if (firstVal == val && secondVal != val && secondVal > firstVal) {
            list.set(0, secondVal);
        }
        int lastVal = list.get(list.size() - 1);
        int secondLastVal = list.get(list.size() - 2);
        if (lastVal == val && secondLastVal != val && secondLastVal > lastVal) {
            list.set(list.size() - 1, secondLastVal);
        }
            ...
            if (list.get(i) == 3) {
                for (int j = 0; j < list.size(); j++) {
                    if (list.get(j) == 4 && (j == 0 || list.get(j - 1) != 3)) {
                        ...
                        list.set(i + 1, 4);
                        list.set(j, temp);
                        break; // Exit inner loop, go to next 3
                    }
                }
            }
        }
     ...
     * no mode and this method should return an empty arraylist.
     *
    ...
    public static ArrayList<Integer> modes(int[] numList) {
        ArrayList<Integer> numbers = new ArrayList<>(); // [1, 2, 3, 4, 5]
        ArrayList<Integer> occurrences = new ArrayList<>(); // [4, 5, 6, 7, 8]
        int greatestOccurrences = 0;
        ...
        for (int num : numList) {
            ...
            if (index == -1) {
                ...
                int newOccurrences = occurrences.get(index) + 1;
                occurrences.set(index, newOccurrences);
                if (newOccurrences > greatestOccurrences) {
                    greatestOccurrences = newOccurrences;
                }
            }
        }

        int leastOccurrences = greatestOccurrences;
        ArrayList<Integer> result = new ArrayList<>();
        for (int i = 0; i < numbers.size(); i++) {
            int currentOccurrences = occurrences.get(i);
            if (currentOccurrences < leastOccurrences) {
                leastOccurrences = currentOccurrences;
            }
            if (currentOccurrences == greatestOccurrences) {
                result.add(numbers.get(i));
            }
        }

        if (leastOccurrences == greatestOccurrences) {
